...
This project is inspired by two key sources:

1. **Soul's Urge Number (Heart's Desire)**: The algorithm calculates the Soul's Urge number, a core concept in numerology, using the vowels in the individual's name and surname. This number is believed to reveal a person's inner motivations, desires, and true self. The calculation follows traditional numerological methods, summing the values of vowels and reducing them to a single digit (digital root).

2. **Magic Square from Aslan Aurziati's Book**: The approach to constructing and interpreting magic squares is based on the teachings and methods described in Aslan Aurziati's book. In this tradition, magic squares are not only mathematical curiosities but also tools for personal insight and spiritual analysis. The algorithm adapts these ideas, embedding personal numerological data into the magic square structure to create a unique matrix for each individual.

By combining these two traditions, the system generates personalized magic squares that serve as the foundation for deep, individualized life path analysis.
