...
Проект вдохновлен двумя ключевыми источниками:

1. **Число Души (Желание Сердца):** Алгоритм вычисляет число души — центральное понятие в нумерологии — используя гласные из имени и фамилии человека. Считается, что это число раскрывает внутренние мотивы, желания и истинную сущность личности. Расчет производится по классическим нумерологическим методам: суммируются значения гласных и приводятся к однозначному числу (цифровой корень).

2. **Магический квадрат по книге Аслана Аурзиати:** Подход к построению и интерпретации магических квадратов основан на методах, описанных в книге Аслана Аурзиати. В этой традиции магические квадраты рассматриваются не только как математические объекты, но и как инструменты для личного и духовного анализа. Алгоритм адаптирует эти идеи, внедряя личные нумерологические данные в структуру квадрата для создания уникальной матрицы для каждого человека.

Объединяя эти две традиции, система создает персонализированные магические квадраты, которые служат основой для глубокого индивидуального анализа жизненного пути.

### Принципы построения магического квадрата (по Аслану Аурзиати)

Согласно книге Аслана Аурзиати, магический квадрат — это квадратная матрица из различных положительных целых чисел, расположенных так, что сумма чисел в каждой строке, каждом столбце и на обеих диагоналях одинакова (магическая константа). В контексте личного анализа выделяются следующие принципы и требования:
...
Этот подход не гарантирует идеальное решение для всех случаев, но эффективно находит качественные персонализированные магические квадраты, соответствующие требованиям Аурзиати.

### Краткий обзор алгоритма
1. **Извлечение личных чисел:**
   - Имя и дата рождения преобразуются в набор "личных чисел" с помощью цифрового корня и кодирования символов.
2. **Генерация магического квадрата:**
   - Для каждого размера (например, 3x3, 7x7, 9x9, 13x13) генерируется популяция случайных квадратов с внедрением личных чисел.
   - Функция пригодности оценивает, насколько хорошо квадрат соответствует магической константе и содержит личные числа.
   - Бустинговый шаг поочередно улучшает квадраты.
   - Используется многопроцессорность для ускорения поиска.
3. **Результат:**
   - Лучшие найденные квадраты сохраняются.
   - Генерируются AI-промпты с этими квадратами и личными данными для создания индивидуальных отчетов.
...
3. **Результат**  
   ...
   - В папке появятся текстовые файлы:
     - `<Ваше Имя>-BRIEF.txt`
     - `<Ваше Имя>_extended_report.txt`
     - `<Ваше Имя>_detailed_report.txt`

### Как использовать результат для AI-промптов
- Каждый сгенерированный `.txt` файл содержит шаблон промпта с вашими данными и магическими квадратами.
- Скопируйте содержимое любого файла и вставьте в выбранную AI-модель (ChatGPT, Claude, Gemini и др.).
- AI сгенерирует детальный индивидуальный отчет на основе ваших квадратов.

#### Пример
1. Откройте `<Имя Фамилия>_extended_report.txt` (или свой файл).
2. Скопируйте все содержимое.
3. Вставьте в ChatGPT или другую LLM и попросите сгенерировать отчет.

### Кастомизация
- Изменяйте размеры квадратов в строке `for size in [3,7,9,13]:` в `magic_square.py`.
